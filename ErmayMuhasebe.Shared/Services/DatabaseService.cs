        ...
        public async Task<DashboardStats> GetDashboardStatsAsync() 
{
    ...
    return stats;
        ...
        private void RegisterRealtimeListener<T>(string resourceName, Func<T, Task> onSave, Func<int, Task> onDelete) where T : class
        {
            var client = _sync.Client;
            if (client == null) 
            {
                System.Diagnostics.Debug.WriteLine($"[DatabaseService] RegisterRealtimeListener: Client is null for {resourceName}");
                return;
            }

            try
            {
                var yearlyPath = _sync.GetYearlyPath(resourceName);
                System.Diagnostics.Debug.WriteLine($"[DatabaseService] RegisterRealtimeListener: Subscribing to path: {yearlyPath}");
                var sub = client
                    .Child(yearlyPath)
                    .AsObservable<T>()
                    .Subscribe(async eventArgs =>
                    {
                        try
                        {
                            System.Diagnostics.Debug.WriteLine($"[DatabaseService] Realtime Event Received: Path={resourceName}, Key={eventArgs.Key}, EventType={eventArgs.EventType}");
                            if (eventArgs.EventType == Firebase.Database.Streaming.FirebaseEventType.Delete)
                            {
                                if (int.TryParse(eventArgs.Key, out int id))
                                {
                                    System.Diagnostics.Debug.WriteLine($"[DatabaseService] Realtime Delete Action: Path={resourceName}, Id={id}");
                                    await onDelete(id);
                                }
                            }
                            else if (eventArgs.Object != null)
                            {
                                var item = eventArgs.Object;
                                var idProp = typeof(T).GetProperty("Id");
                                int id = 0;
                                if (int.TryParse(eventArgs.Key, out int parsedId) && parsedId > 0)
                                {
                                    id = parsedId;
                                ...
                                {
                                    var existingVal = idProp.GetValue(item);
                                    if (existingVal is int currentVal && currentVal > 0)
                                    {
                                        id = currentVal;
                                    }
                                }

                                System.Diagnostics.Debug.WriteLine($"[DatabaseService] Realtime Put/Update Action: Path={resourceName}, Key={eventArgs.Key}, MappedId={id}");
                                await onSave(item);
                            }
                        }
                        catch (Exception ex)
                        {
                            System.Diagnostics.Debug.WriteLine($"[DatabaseService] Error handling realtime event for {resourceName}: {ex.Message}");
                        }
                    }, ex =>
                    {
                        System.Diagnostics.Debug.WriteLine($"[DatabaseService] Realtime Listener Subscription Error for {resourceName}: {ex.Message}");
                    });
                _realtimeSubscriptions.Add(sub);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[DatabaseService] RegisterRealtimeListener Error for {resourceName}: {ex.Message}");
            }
        }

        public void StopRealtimeSync()
        {
            foreach (var sub in _realtimeSubscriptions)
            {
                sub.Dispose();
            }
            _realtimeSubscriptions.Clear();
            System.Diagnostics.Debug.WriteLine("[DatabaseService] Realtime Sync STOPPED.");
        }
        ...
        public void StartRealtimeSync()
        {
            ...
            {
                // Register listeners
                RegisterRealtimeListener<CariKart>("Cariler", async item => {
                    var existing = await _db.Table<CariKart>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    var existing = await _db.Table<CariKart>().FirstOrDefaultAsync(x => x.Id == id);
                ...
                RegisterRealtimeListener<CariHareket>("CariHareketler", async item => {
                    ...
                    await _db.DeleteAsync<CariHareket>(id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<StokKart>("Stoklar", async item => {
                    var existing = await _db.Table<StokKart>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    var existing = await _db.Table<StokKart>().FirstOrDefaultAsync(x => x.Id == id);
                    if (existing != null && !existing.IsDeleted)
                    {
                        existing.IsDeleted = true;
                        await _db.UpdateAsync(existing);
                ...
                RegisterRealtimeListener<StokGrupDef>("StokGruplar", async item => {
                    var existing = await _db.Table<StokGrupDef>().FirstOrDefaultAsync(x => x.Id == item.Id || (x.Ad == item.Ad && !x.IsDeleted));
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    var existing = await _db.Table<StokGrupDef>().FirstOrDefaultAsync(x => x.Id == id);
                    if (existing != null && !existing.IsDeleted)
                    {
                        existing.IsDeleted = true;
                        await _db.UpdateAsync(existing);
                ...
                RegisterRealtimeListener<StokHareket>("StokHareketler", async item => {
                    var existing = await _db.Table<StokHareket>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    await _db.DeleteAsync<StokHareket>(id);
                    NotifyDatabaseChanged();
                });
                ...
                RegisterRealtimeListener<Fatura>("Faturalar", async item => {
                    ...
                    if (changed)
                    {
                        var details = await _sync.PullFaturaDetaylarAsync(item.Id);
                ...
                RegisterRealtimeListener<BankaKart>("Bankalar", async item => {
                    var existing = await _db.Table<BankaKart>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    await _db.DeleteAsync<BankaKart>(id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<KasaHareket>("KasaHareketler", async item => {
                    var existing = await _db.Table<KasaHareket>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    await _db.DeleteAsync<KasaHareket>(id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<BankaHareket>("BankaHareketler", async item => {
                    var existing = await _db.Table<BankaHareket>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    await _db.DeleteAsync<BankaHareket>(id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<Cek>("Cekler", async item => {
                    var existing = await _db.Table<Cek>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    await _db.DeleteAsync<Cek>(id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<Senet>("Senetler", async item => {
                    var existing = await _db.Table<Senet>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    await _db.DeleteAsync<Senet>(id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<Siparis>("Siparisler", async item => {
                    var existing = await _db.Table<Siparis>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    bool changed = false;
                    if (existing == null)
                    {
                        await _db.InsertAsync(item);
                    ...
                    if (changed)
                    {
                        var details = await _sync.PullSiparisDetaylarAsync(item.Id);
                        if (details != null && details.Count > 0)
                        {
                            await _db.ExecuteAsync("DELETE FROM SiparisDetay WHERE SiparisId = ?", item.Id);
                            foreach (var d in details)
                            {
                                if (d == null) continue;
                                await _db.InsertAsync(d);
                    ...
                    await _db.ExecuteAsync("DELETE FROM Siparis WHERE Id = ?", id);
                    await _db.ExecuteAsync("DELETE FROM SiparisDetay WHERE SiparisId = ?", id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<Teklif>("Teklifler", async item => {
                    var existing = await _db.Table<Teklif>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    bool changed = false;
                    if (existing == null)
                    {
                        await _db.InsertAsync(item);
                    ...
                    if (changed)
                    {
                        var details = await _sync.PullTeklifDetaylarAsync(item.Id);
                        if (details != null && details.Count > 0)
                        {
                            await _db.ExecuteAsync("DELETE FROM TeklifDetay WHERE TeklifId = ?", item.Id);
                            foreach (var d in details)
                            {
                                if (d == null) continue;
                                await _db.InsertAsync(d);
                    ...
                    await _db.ExecuteAsync("DELETE FROM Teklif WHERE Id = ?", id);
                    await _db.ExecuteAsync("DELETE FROM TeklifDetay WHERE TeklifId = ?", id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<Note>("Notes", async item => {
                    var existing = await _db.Table<Note>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                ...
                RegisterRealtimeListener<KrediKartiIslem>("KrediKartlari", async item => {
                    var existing = await _db.Table<KrediKartiIslem>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    await _db.DeleteAsync<KrediKartiIslem>(id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<EftIslem>("EftIslemleri", async item => {
                    var existing = await _db.Table<EftIslem>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    await _db.DeleteAsync<EftIslem>(id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<Gorev>("Gorevler", async item => {
                    var existing = await _db.Table<Gorev>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    await _db.DeleteAsync<Gorev>(id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<Personel>("Personeller", async item => {
                    var existing = await _db.Table<Personel>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    await _db.DeleteAsync<Personel>(id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<SatisHedefi>("SatisHedefleri", async item => {
                    var existing = await _db.Table<SatisHedefi>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                ...
                RegisterRealtimeListener<HaftalikSatisHedefi>("HaftalikSatisHedefleri", async item => {
                    var existing = await _db.Table<HaftalikSatisHedefi>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                ...
                RegisterRealtimeListener<YillikSatisHedefi>("YillikSatisHedefleri", async item => {
                    var existing = await _db.Table<YillikSatisHedefi>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                ...
                RegisterRealtimeListener<StokSayimFisi>("StokSayimlar", async item => {
                    var existing = await _db.Table<StokSayimFisi>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    bool changed = false;
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                    ...
                    if (changed)
                    {
                        var details = await _sync.PullStokSayimDetaylarAsync(item.Id);
                        if (details != null && details.Count > 0)
                        {
                            await _db.ExecuteAsync("DELETE FROM StokSayimDetay WHERE FisId = ?", item.Id);
                            foreach (var d in details)
                            {
                                if (d == null) continue;
                                await _db.InsertAsync(d);
                    ...
                    await _db.ExecuteAsync("DELETE FROM StokSayimFisi WHERE Id = ?", id);
                    await _db.ExecuteAsync("DELETE FROM StokSayimDetay WHERE FisId = ?", id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<PortfoyKart>("PortfoyKartlari", async item => {
                    var existing = await _db.Table<PortfoyKart>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    await _db.DeleteAsync<PortfoyKart>(id);
                    NotifyDatabaseChanged();
                });

                RegisterRealtimeListener<MusteriTakipKlasor>("MusteriTakipKlasorler", async item => {
                    var existing = await _db.Table<MusteriTakipKlasor>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    var existing = await _db.Table<MusteriTakipKlasor>().FirstOrDefaultAsync(x => x.Id == id);
                    if (existing != null)
                    {
                        existing.IsDeleted = true;
                        await _db.UpdateAsync(existing);
                ...
                RegisterRealtimeListener<MusteriTakipDetay>("MusteriTakipDetaylar", async item => {
                    var existing = await _db.Table<MusteriTakipDetay>().FirstOrDefaultAsync(x => x.Id == item.Id);
                    if (existing == null) 
                    {
                        await _db.InsertAsync(item);
                        NotifyDatabaseChanged();
                    ...
                    var existing = await _db.Table<MusteriTakipDetay>().FirstOrDefaultAsync(x => x.Id == id);
                    if (existing != null)
                    {
                        existing.IsDeleted = true;
                        await _db.UpdateAsync(existing);
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[DatabaseService] Realtime Sync Start Error: {ex.Message}");
            }
        }
